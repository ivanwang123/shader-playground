...
void main() {
  ...
  vec2 uvs[4];
  uvs[0] = vec2(vUv.x, vUv.y + uResolution.w * thickness);
  uvs[1] = vec2(vUv.x, vUv.y - uResolution.w * thickness);
  uvs[2] = vec2(vUv.x + uResolution.z * thickness, vUv.y);
  uvs[3] = vec2(vUv.x - uResolution.z * thickness, vUv.y);
