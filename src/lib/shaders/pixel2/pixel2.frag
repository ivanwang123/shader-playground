...
float getDepth(int x, int y) {
  return texture2D(tDepth, vUv + vec2(x, y) * uResolution.zw).r;
}

vec3 getNormal(int x, int y) {
  return texture2D(tNormal, vUv + vec2(x, y) * uResolution.zw).rgb * 2.0 - 1.0;
}
...
void main() {
  ...
  // gl_FragColor = vec4(vec3(coefficient), 1.0);
